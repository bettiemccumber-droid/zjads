...
/** 区间内仍无 Performance 日数据的商家 MID */
function listMidsMissingDailyMetrics_(
  agg: Map<string, RwMerchantClickAgg>,
  merchantIds: string[],
  startDate: string,
  endDate: string,
  ...
  for (const mid of merchantIds) {
    ...
    for (const row of agg.values()) {
      if (row.merchantId !== mid) continue;
      if (row.clickDate < startDate || row.clickDate > endDate) continue;
      if (row.clicks > 0 || row.performanceOrders > 0) {
        has = true;
        break;
      }
    }
 ...
 */
async function fetchRwBulkMerchantDayMatrix_(
  apiToken: string,
  startDate: string,
  endDate: string,
  agg: Map<string, RwMerchantClickAgg>,
  ...
  for (const row of bulkCpc.values()) {
    mergeRwAggRow_(agg, row);
  }
  if (bulkCpc.size > 0) {
    const merchantCount = new Set([...bulkCpc.values()].map((r) => r.merchantId)).size;
    await onProgress?.(
      `bulk cpc → ${merchantCount} 商家 / ${sumAggPerformanceOrders_(agg)} 单 / ${sumAggClicks_(agg)} 点击`,
    );
    if (
  ...
  for (const spec of RW_PERFORMANCE_ORDER_SOURCES) {
    ...
    if (hasAggMetrics_(bulkPerf)) break;
  }
  for (const row of bulkPerf.values()) {
    mergeRwAggRow_(agg, row);
  }
  if (hasAggMetrics_(bulkPerf)) {
    const merchantCount = new Set([...bulkPerf.values()].map((r) => r.merchantId)).size;
    await onProgress?.(
      `bulk performance → ${merchantCount} 商家 / ${sumAggPerformanceOrders_(agg)} 单 / ${sumAggClicks_(agg)} 点击`,
    );
    return true;
  }
...
/**
 * 逐商家拉取按日 Performance（等同后台先筛 Merchant 再 Group by Daily，自动化批量）
 */
async function fetchRwPerMerchantDailyMetrics_(
  apiToken: string,
  startDate: string,
  endDate: string,
  merchantIds: string[],
  ...
  const unique = [...new Set(merchantIds.filter((id) => id.trim() !== ''))].slice(
    0,
    RW_PER_MERCHANT_DAILY_MAX,
  );
  const out: RwMerchantClickAgg[] = [];
  ...
  for (let i = 0; i < unique.length; i += 1) {
    ...
    if (i === 0 || (i + 1) % 10 === 0 || i === unique.length - 1) {
      const clickSum = out.reduce((s, r) => s + r.clicks, 0);
      await onProgress?.(
        `逐商家按日 ${i + 1}/${unique.length}（mid=${mid}）累计 ${clickSum} 点击`,
      );
    }
  }
...
/** 合并账号级 Performance 按日行；可选仅归因到单一商家 */
function mergeRwAccountDailyMetricsRow_(
  ...
  if (merchantId && targetMid && merchantId !== targetMid) return;
  if (merchantId && !targetMid) return;
...
async function tryFetchPerformanceReportDaily_(
  ...
  for (const params of paramVariants) {
    ...
    if (agg.size === 0 && result.rowCount === 0) {
      await onProgress?.(
        `performance/report GET code=${result.code} rows=0`,
      );
      /** 部分站点仅支持 POST */
      const postAgg = new Map<string, RwMerchantClickAgg>();
      try {
      ...
      for (const row of postAgg.values()) {
        if (row.performanceOrders <= 0 && row.clicks <= 0) continue;
        const clickDate = row.clickDate;
        if (!clickDate || clickDate < startDate || clickDate > endDate) continue;
        let merchantId = row.merchantId;
        if (!merchantId || isRwClickPseudoMerchant(merchantId)) {
          const midsOnDay = merchantsByDate.get(clickDate);
          if (!midsOnDay || midsOnDay.size !== 1) continue;
          merchantId = [...midsOnDay][0]!;
        }
...
/** 将 user_click 未解析商家的点击归因到当日唯一有佣金的商家 */
function attributeRwUnmatchedClicks_(
  clickAgg: Map<string, RwMerchantClickAgg>,
  mids: string[],
  merchantsByDate: Map<string, Set<string>>,
): void {
  for (const [key, row] of [...clickAgg.entries()]) {
    if (!isRwClickPseudoMerchant(row.merchantId)) continue;
    if (row.clicks <= 0) continue;

    const day = row.clickDate;
    let targetMid: string | undefined;
    const dayMids = merchantsByDate.get(day);
    if (dayMids?.size === 1) {
      targetMid = [...dayMids][0];
    ...
    const toKey = `${targetMid}|${day}`;
    const existing = clickAgg.get(toKey);
    if (existing) {
...
/** GET 分页上限（补充链路用，避免空数据时翻页过久） */
const RW_SUPPLEMENT_MAX_PAGES = 3;

/** bulk 商家×按日矩阵允许更多分页 */
const RW_BULK_MAX_PAGES = 30;

/**
 * 按自然日拉取 orders/clicks（快速路径优先，90s 预算，命中即停）
 */
export async function fetchRewardooPerformanceDailyAggs(
  ...
  const needClicks = () => {
    if (!includeClicks) return false;
    if (sumAggClicks_(agg) === 0) return true;
    if (mids.length === 0) return false;
    return listMidsMissingDailyMetrics_(agg, mids, startDate, endDate).length > 0;
  };
  const isSatisfied = () =>
    includeClicks ? !needClicks() && !needOrders() : !needOrders() && !needClicks();

  const mergeRows = (rows: RwMerchantClickAgg[], restrictToMids = true) => {
    for (const row of rows) {
      if (restrictToMids && mids.length > 0 && !mids.includes(row.merchantId)) continue;
      if (row.performanceOrders <= 0 && row.clicks <= 0) continue;
      mergeRwAggRow_(agg, row);
    }
  };
  ...
  if (!skipOrderFetch && !isSatisfied() && !timedOut()) {
    ...
    if (perfRows.length > 0) {
      await onProgress?.(
        `逐日 Performance → ${sumAggPerformanceOrders_(agg)} 单 / ${sumAggClicks_(agg)} 点击`,
      );
    }
  }

  /** 2. 联盟点击汇总接口（medium/performance 整段） */
  if (needClicks() && !timedOut()) {
    await onProgress?.('RW Performance：汇总点击接口…');
    const clickRows = await fetchRewardooClicks(apiToken, startDate, endDate, async (p) => {
      await onProgress?.(
        `${p.source ?? 'click'} ${p.slotIndex}/${p.totalSlots}，已汇总 ${p.clicksSoFar} 点击`,
      );
    });
    for (const row of clickRows) {
      ...
      if (row.clicks <= 0 && row.performanceOrders <= 0) continue;
      mergeRwAggRow_(agg, {
        ...row,
        performanceCommission: 0,
      });
  ...
  /** 3. 逐商家按日（等同后台筛 Merchant + Daily，批量自动，仅补 bulk 未覆盖的 MID） */
  if ((needOrders() || needClicks()) && !timedOut() && mids.length > 0) {
    const missingMids = listMidsMissingDailyMetrics_(agg, mids, startDate, endDate);
    if (missingMids.length > 0) {
      await onProgress?.(
        `RW Performance：逐商家按日补漏 ${missingMids.length} 个（最多 ${RW_PER_MERCHANT_DAILY_MAX}）…`,
      );
      const perMerchantRows = await fetchRwPerMerchantDailyMetrics_(
        apiToken,
        startDate,
        endDate,
        missingMids,
      ...
      mergeRows(perMerchantRows, true);
    }
  }

  /** 4. performance/report 区间按日 */
  ...
  /** 5. cpc_performance 区间 + 商家维度（按 mid 单次 GET/POST） */
  if (!isSatisfied() && !timedOut()) {
    await onProgress?.('RW Performance：cpc_performance 按日+商家…');
    const targetMids =
      mids.length === 1
        ? mids
        : [...new Set([...merchantsByDate.values()].flatMap((s) => [...s]))];
    const tryMids: Array<string | undefined> =
      targetMids.length > 0 ? targetMids : [undefined];

    for (const mid of tryMids) {
      if (timedOut() || isSatisfied()) break;
      const got = await fetchRwCpcPerformanceDailyQuick_(
        apiToken,
        startDate,
        endDate,
        agg,
      ...
      if (got) {
        await onProgress?.(
          `cpc_performance${mid ? ` mid=${mid}` : ''} → ${sumAggPerformanceOrders_(agg)} 单 / ${sumAggClicks_(agg)} 点击`,
        );
        break;
      }
    }
  ...
  /** 6. medium/performance 区间（仅 orders 仍缺时） */
  ...
  if (needClicks() && !timedOut()) {
    ...
    const gotClicks = await fetchRwUserClickAggs_(
      ...
      async (p) => {
        await onProgress?.(
          `user_click ${p.slotIndex}/${p.totalSlots} 天，已汇总 ${p.clicksSoFar} 点击`,
        );
      },
    );
    if (gotClicks) {
      attributeRwUnmatchedClicks_(clickAgg, mids, merchantsByDate);
  ...
  /** 8. medium/performance 逐日 GET 兜底 */
  if (!isSatisfied() && !timedOut()) {
  ...
  if (timedOut()) {
    await onProgress?.(
      `Performance 补充超时（${RW_PERF_SUPPLEMENT_BUDGET_MS / 1000}s）→ ${orderTotal} 单 / ${clickTotal} 点击`,
    );
  } else if (orderTotal > 0 || clickTotal > 0) {
    await onProgress?.(
...
/** cpc_performance 区间按日+商家（单次 GET，补充链路专用） */
async function fetchRwCpcPerformanceDailyQuick_(
  apiToken: string,
  startDate: string,
  endDate: string,
  agg: Map<string, RwMerchantClickAgg>,
  ...
  const params: Record<string, string> = {
    ...
    sub_dimension: 'merchant',
    status: 'All',
    ...(forcedMid ? rwMidParams_(forcedMid) : {}),
  };
    ...
    (rows) => {
      for (const raw of rows) {
        mergeSummaryClickRow_(
          ...
          endDate,
          undefined,
          { rwPerformanceDaily: true, forcedMid },
        );
      }
    },
    RW_CLICK_PAGE_SIZE,
  ...
  let got =
    sumAggPerformanceOrders_(agg) > ordersBefore || sumAggClicks_(agg) > clicksBefore;
...
async function fetchRwPerformanceDaily_(
  ...
  const trySources = async (
    ...
    const sourceLists = [RW_CPS_DAILY_SOURCES, RW_PERFORMANCE_ORDER_SOURCES];
